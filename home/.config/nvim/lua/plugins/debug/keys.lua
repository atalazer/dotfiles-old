local k = vim.keymap
local nnoremap = k.nnoremap

local dap = require("dap")
local ui = require("dapui")
local api = vim.api

local keymap_restore = {}

nnoremap({ "<Leader>dd", dap.toggle_breakpoint, { silent = true } })
nnoremap({ "<Leader>dc", dap.continue, { silent = true } })
nnoremap({ "<Leader>do", dap.step_over, { silent = true } })
nnoremap({ "<Leader>d>", dap.step_into, { silent = true } })
nnoremap({ "<Leader>d<", dap.step_out, { silent = true } })
nnoremap({ "<Leader>dr", dap.repl.open, { silent = true } })
nnoremap({ "<Leader>da", ui.toggle, { silent = true } })
nnoremap({ "<Leader>de", ui.eval, { silent = true } })

if vim.bo.filetype == "lua" then
    api.nvim_buf_set_keymap("n", "<Leader>dl", ":lua require'osv'.run_this()<CR>", { noremap = true, silent = true })
end

if vim.bo.filetype == "python" then
    api.nvim_buf_set_keymap(
        "n",
        "<Leader>dm",
        ":lua require('dap-python').test_method()<CR>",
        { noremap = true, silent = true }
    )
    api.nvim_buf_set_keymap(
        "n",
        "<Leader>dn",
        ":lua require('dap-python').test_class()<CR>",
        { noremap = true, silent = true }
    )
    api.nvim_buf_set_keymap(
        "n",
        "<Leader>ds",
        "<ESC>:lua require('dap-python').debug_selection()<CR>",
        { noremap = true, silent = true }
    )
end

dap.listeners.after["event_initialized"]["me"] = function()
    for _, buf in pairs(api.nvim_list_bufs()) do
        local keymaps = api.nvim_buf_get_keymap(buf, "n")
        for _, keymap in pairs(keymaps) do
            if keymap.lhs == "K" then
                table.insert(keymap_restore, keymap)
                api.nvim_buf_del_keymap(buf, "n", "K")
            end
        end
    end
    nnoremap({
        "K",
        function()
            require("dap.ui.widgets").hover(nil, { border = Util.borders })
        end,
    })
end

dap.listeners.after["event_terminated"]["me"] = function()
    for _, keymap in pairs(keymap_restore) do
        api.nvim_buf_set_keymap(keymap.buffer, keymap.mode, keymap.lhs, keymap.rhs, { silent = keymap.silent == 1 })
    end
    keymap_restore = {}
end
