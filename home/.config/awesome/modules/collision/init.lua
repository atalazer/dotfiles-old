...
local col_utils    = require( "modules.collision.util" )
local unpack = unpack or table.unpack -- luacheck: globals unpack (compatibility with Lua 5.1)
local module = {
  _focus  = require( "modules.collision.focus" ),
  _resize = require( "modules.collision.resize"),
  _max    = require( "modules.collision.max"   ),
  _screen = require( "modules.collision.screen"),
  mouse   = require( "modules.collision.mouse" ),
  ...
  glib.idle_add(glib.PRIORITY_DEFAULT_IDLE, function()
    for k,v in pairs(keys) do
      for _,key_name in ipairs(v) do
        aw[#aw+1] = awful.key({ "Mod4" }, 
                        key_name, function () module.focus (k          ) end,
                    { description = "Change focus to the "..key_name, group = "Collision" })
        --aw[#aw+1] = awful.key({ "Mod4", "Shift"},
                        --key_name, function () module.resize(k          ) end,
                    --{ description = "Resize to the "..key_name, group = "Collision" })
        aw[#aw+1] = awful.key({ "Mod4", "Control"}, 
                        key_name, function () module.move  (k          ) end,
                    { description = "Move to the "..key_name, group = "Collision" })
        --aw[#aw+1] = awful.key({ "Mod4", "Shift",   "Control"         }, 
                        --key_name, function () module.move  (k,nil ,true) end,
                    --{ description = "", group = "Collision" })
        --aw[#aw+1] = awful.key({ "Mod4",            "Control"         }, 
                        --key_name, function () module.focus (k,nil ,true) end,
                    --{ description = "Change floating focus to the "..key_name, group = "Collision" })
        --aw[#aw+1] = awful.key({ "Mod4", "Mod1" ,   "Control"         }, 
                        --key_name, function () module.screen(k          ) end,
                    --{ description = "Change screen to the "..key_name, group = "Collision" })
        --aw[#aw+1] = awful.key({ "Mod1", "Shift", "Control", "Mod4" }, 
                        --key_name, function () module.screen(k,true     ) end,
        ...
        if k == "left" or k =="right" then -- Conflict with my text editor, so I say no
          aw[#aw+1] =   awful.key({ "Mod4", "Mod1" }, 
                            key_name, function () module.tag   (k,nil ,true) end,
                        { description = "Select tag to the "..key_name, group = "Collision" })
          --aw[#aw+1] =   awful.key({ "Mod4", "Shift", "Control"         }, 
                            --key_name, function () module.tag   (k,true,true) end,
